...
module FFIGenerate
  class Generator
    ...
    AUTO_GENERATED_HEADER = "# [generated by ffi_generator] #"
    ...
    def initialize(options = {})
      ...
      @ffi_lib_path_env = ffi_lib_path_env
    ...
    def ffi_lib_path_env
      flat_name = @module_name.upcase.gsub("::", "_")
      "#{flat_name}_PATH".gsub(/[\W]+/, "")
    end
    ...
    def generate_rb
      ...
      writer.puts AUTO_GENERATED_HEADER, ""
      writer.puts "require 'ffi'", ""

      write_modules_block(modules) do
        write_header
        declarations.each(&:write_ruby)
      end

      writer.output
    end

    def write_modules_block(modules, &block)
      write_one_module_block(modules, 0, modules.length, &block)
    end

    def write_one_module_block(modules, index, length, &block)
      writer.puts "module #{modules[index]}"
      writer.indent do
        if index < length - 1
          write_one_module_block(modules, index + 1, length, &block)
        else
          yield
        end
      end
    ...
    def write_header
      ...
      if @ffi_lib
        msg = "ffi_lib ENV[\"#{@ffi_lib_path_env}\"] || #{@ffi_lib.inspect}"
        puts "Setting lib path as: #{msg}"
        writer.puts msg, ""
      end

      write_attach_function
    end

    def write_attach_function
      writer.puts \
